            ...
            if (proc_st != ProcessState::IDLE && 
                ...
                switch (currentUiState) {
                    case UiState::UI_STATE_IDLE:
                        ...
                        // NOWE: Sprawdzenie przycisku DOWN w trybie AUTO dla przejścia do następnego kroku
                    ...
                    // NOWY STAN: Potwierdzenie przejścia do następnego kroku
                    ...
                    case UiState::UI_STATE_DIAGNOSTICS:
                        if (pin == PIN_BTN_EXIT) { 
                            ...
                            buzzerBeep(1, 30, 0);
                            // Możesz dodac przewijanie informacji diagnostycznych
                        }
                        break;
                }
            }
...
void ui_update_display() {
    ...
    // Tło i podstawowe etykiety (tylko jeśli potrzebne)
    if (force_redraw || displayCache.needsRedraw) {
    ...
    // Czyszczenie dolnej części ekranu przy zmianie stanu UI
    ...
    if (currentUiState == UiState::UI_STATE_IDLE) {
        if (st != ProcessState::IDLE) {
            ...
            if (st == ProcessState::RUNNING_AUTO) {
                ...
                // DODANE: Instrukcje z ikonami dla trybu AUTO
                printCustom(display, 5, 130, "[", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 17, 130, "] Nastepny krok", ST77XX_CYAN, ST77XX_BLACK, 1);
                
                printCustom(display, 5, 145, "[", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 17, 145, "] Zatrzymaj", ST77XX_CYAN, ST77XX_BLACK, 1);
            ...
            display.print("Menu");
            printCustom(display, 25, 115, "[", ST77XX_GREEN, ST77XX_BLACK, 2);
            printCustom(display, 45, 115, "]", ST77XX_GREEN, ST77XX_BLACK, 2);
        }
    } else {
        display.setTextSize(1);
        switch (currentUiState) {
            case UiState::UI_STATE_MENU_MAIN:
                ...
                // Dodaj ikony nawigacji
                printCustom(display, 10, 145, "[", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 22, 145, "] Wybierz  [", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 75, 145, "] OK  [", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 105, 145, "] Powrot", ST77XX_CYAN, ST77XX_BLACK, 1);
                break;
                
            case UiState::UI_STATE_MENU_SOURCE:
                display.setCursor(10, 90); 
                display.setTextColor(sourceMenuIndex == 0 ? ST77XX_GREEN : ST77XX_WHITE); 
                printCustom(display, 25, 90, "Karta SD", sourceMenuIndex == 0 ? ST77XX_GREEN : ST77XX_WHITE, ST77XX_BLACK, 1);
                display.setCursor(10, 115); 
                display.setTextColor(sourceMenuIndex == 1 ? ST77XX_GREEN : ST77XX_WHITE); 
                printCustom(display, 25, 115, "GitHub", sourceMenuIndex == 1 ? ST77XX_GREEN : ST77XX_WHITE, ST77XX_BLACK, 1);
                
                printCustom(display, 10, 145, "[", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 22, 145, "]  [", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 40, 145, "]  [", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 58, 145, "]", ST77XX_CYAN, ST77XX_BLACK, 1);
                break;
                
            case UiState::UI_STATE_MENU_PROFILES:
                if (profilesLoading) {
                    printCustom(display, 10, 95, "Wczytywanie...", ST77XX_YELLOW, ST77XX_BLACK, 1);
                ...
                if (profileList.empty()) {
                    printCustom(display, 10, 95, "Brak profili!", ST77XX_RED, ST77XX_BLACK, 1);
                } else {
                    display.setTextSize(1);
                ...
                printCustom(display, 10, 145, "[", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 22, 145, "]  [", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 40, 145, "] Wybierz  [", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 95, 145, "]", ST77XX_CYAN, ST77XX_BLACK, 1);
                break;
            ...
            case UiState::UI_STATE_EDIT_MANUAL:
                ...
                printCustom(display, 12, 80, "Temp: " + String(ts, 1) + " C", manualEditIndex == 0 ? ST77XX_YELLOW : ST77XX_WHITE, ST77XX_BLACK, 1);
                display.setCursor(0, 92); 
                display.setTextColor(manualEditIndex == 1 ? ST77XX_YELLOW : ST77XX_WHITE); 
                printCustom(display, 12, 92, "Moc: " + String(pm), manualEditIndex == 1 ? ST77XX_YELLOW : ST77XX_WHITE, ST77XX_BLACK, 1);
                display.setCursor(0, 104); 
                display.setTextColor(manualEditIndex == 2 ? ST77XX_YELLOW : ST77XX_WHITE); 
                printCustom(display, 12, 104, "Dym: " + String(smoke), manualEditIndex == 2 ? ST77XX_YELLOW : ST77XX_WHITE, ST77XX_BLACK, 1);
                display.setCursor(0, 116); 
                display.setTextColor(manualEditIndex == 3 ? ST77XX_YELLOW : ST77XX_WHITE);
                if(fm == 0) printCustom(display, 12, 116, "Went: OFF", manualEditIndex == 3 ? ST77XX_YELLOW : ST77XX_WHITE, ST77XX_BLACK, 1);
                else if (fm == 1) printCustom(display, 12, 116, "Went: ON", manualEditIndex == 3 ? ST77XX_YELLOW : ST77XX_WHITE, ST77XX_BLACK, 1);
                else printCustom(display, 12, 116, "Went: CYKL", manualEditIndex == 3 ? ST77XX_YELLOW : ST77XX_WHITE, ST77XX_BLACK, 1);
                display.setTextSize(2);
                printCustom(display, 15, 135, "[", manualEditIndex == 4 ? ST77XX_YELLOW : ST77XX_WHITE, ST77XX_BLACK, 2);
                printCustom(display, 35, 135, "]", manualEditIndex == 4 ? ST77XX_YELLOW : ST77XX_WHITE, ST77XX_BLACK, 2);
                
                // Dodaj legendę klawiszy
                printCustom(display, 0, 150, "[", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 12, 150, "] Zmien  [", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 55, 150, "] Dalej  [", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 90, 150, "] Powrot", ST77XX_CYAN, ST77XX_BLACK, 1);
                break;
                
            case UiState::UI_STATE_CONFIRM_ACTION:
                printCustom(display, 15, 95, "Na pewno?", ST77XX_WHITE, ST77XX_BLACK, 1);
                display.setTextSize(1);
                display.setCursor(10, 120); 
                display.setTextColor(!confirmSelection ? ST77XX_GREEN : ST77XX_WHITE); 
                printCustom(display, 12, 120, "NIE", !confirmSelection ? ST77XX_GREEN : ST77XX_WHITE, ST77XX_BLACK, 1);
                display.setCursor(70, 120); 
                display.setTextColor(confirmSelection ? ST77XX_GREEN : ST77XX_WHITE); 
                printCustom(display, 72, 120, "TAK", confirmSelection ? ST77XX_GREEN : ST77XX_WHITE, ST77XX_BLACK, 1);
                
                printCustom(display, 10, 145, "[", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 22, 145, "] Wybierz  [", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 75, 145, "] OK  [", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 105, 145, "] Anuluj", ST77XX_CYAN, ST77XX_BLACK, 1);
                break;
                
            // NOWY EKRAN: Potwierdzenie przejścia do następnego kroku
            case UiState::UI_STATE_CONFIRM_NEXT_STEP:
                printCustom(display, 10, 85, "Nastepny krok?", ST77XX_YELLOW, ST77XX_BLACK, 1);
                printCustom(display, 10, 100, "Pominac biezacy krok?", ST77XX_WHITE, ST77XX_BLACK, 1);
                display.setCursor(10, 120); 
                display.setTextColor(!confirmSelection ? ST77XX_GREEN : ST77XX_WHITE); 
                printCustom(display, 12, 120, "NIE", !confirmSelection ? ST77XX_GREEN : ST77XX_WHITE, ST77XX_BLACK, 1);
                display.setCursor(70, 120); 
            ...
            case UiState::UI_STATE_SYSTEM_SETTINGS:
                {
                    printCustom(display, 10, 75, "USTAWIENIA SYSTEMU", ST77XX_WHITE, ST77XX_BLACK, 1);
                    ...
                    printCustom(display, 5, 145, "[", ST77XX_CYAN, ST77XX_BLACK, 1);
                    printCustom(display, 17, 145, "] Wybierz  [", ST77XX_CYAN, ST77XX_BLACK, 1);
                    printCustom(display, 70, 145, "] OK  [", ST77XX_CYAN, ST77XX_BLACK, 1);
                    printCustom(display, 100, 145, "] Powrot", ST77XX_CYAN, ST77XX_BLACK, 1);
                }
                break;
                
            case UiState::UI_STATE_WIFI_SETTINGS:
                {
                    printCustom(display, 10, 75, "USTAWIENIA WiFi", ST77XX_WHITE, ST77XX_BLACK, 1);
                    display.drawFastHLine(10, 85, 108, ST77XX_WHITE);
                    ...
                    printCustom(display, 5, 145, "[", ST77XX_CYAN, ST77XX_BLACK, 1);
                    printCustom(display, 17, 145, "] Wybierz  [", ST77XX_CYAN, ST77XX_BLACK, 1);
                    printCustom(display, 70, 145, "] Powrot", ST77XX_CYAN, ST77XX_BLACK, 1);
                }
                break;
                
            case UiState::UI_STATE_DIAGNOSTICS:
                showDiagnosticsScreen();
                printCustom(display, 10, 150, "[", ST77XX_CYAN, ST77XX_BLACK, 1);
                printCustom(display, 22, 150, "] Powrot", ST77XX_CYAN, ST77XX_BLACK, 1);
                break;
        }
    }
...
bool shouldEnterLowPowerMode() {
    ...
    // Nie przechodź w tryb niskiego poboru jesli jestesmy w menu
