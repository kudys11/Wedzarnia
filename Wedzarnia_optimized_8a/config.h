// config.h - Zoptymalizowana wersja ze stabilnymi timeoutami
#pragma once
#include <Arduino.h>
#include <WiFi.h>
#include <SD.h>
...
#define PIN_DOOR 25// config.h - Zoptymalizowana wersja ze stabilnymi timeoutami
#pragma once
#include <Arduino.h>
#include <WiFi.h>
#include <SD.h>
...
// --- Watchdog ---
// [FIX] Zwiększony z 8 do 10s - DS18B20 konwersja + odczyt zajmuje ~2s
constexpr int WDT_TIMEOUT = 10;
constexpr unsigned long SOFT_WDT_TIMEOUT = 30000;
// [FIX] Zwiększony z 2s do 10s - zbyt krótki powodował fałszywe alarmy
...
constexpr unsigned long SENSOR_READ_TIMEOUT = 100; // ms timeout dla czujników

// --- Stałe przypisania czujników ---
constexpr int DEFAULT_CHAMBER_SENSOR = 0;
constexpr int DEFAULT_MEAT_SENSOR = 1;
...
// --- Adaptive PID ---
constexpr unsigned long PID_ADAPTATION_INTERVAL = 60000; // 1 minuta
...
// ======================================================
// [NEW] ZABEZPIECZENIE: GRZAŁKA BEZ WZROSTU TEMPERATURY
// ======================================================
// Czas ciągłego grzania bez wymaganego wzrostu temp → awaria
constexpr unsigned long HEATER_NO_RISE_TIMEOUT_MS = 20UL * 60UL * 1000UL; // 20 minut

// Minimalny wzrost temperatury wymagany w oknie HEATER_NO_RISE_TIMEOUT_MS
constexpr double HEATER_MIN_TEMP_RISE = 2.0; // °C

// Minimalne wyjście PID uznawane za "aktywne grzanie"
constexpr double HEATER_FAULT_MIN_PID = 50.0; // %

// Minimalna różnica temp. komory od setpointu, przy której monitoring jest aktywny.
// Gdy temp. jest blisko celu, PID celowo redukuje moc – nie sprawdzamy wtedy awarii.
constexpr double HEATER_FAULT_MIN_ERROR = 10.0; // °C poniżej setpointu
...
enum class ProcessState {
    ...
    ERROR_PROFILE,
    SOFT_RESUME,
    PAUSE_HEATER_FAULT   // [NEW] grzałka włączona, ale brak wzrostu temperatury
};
...
// [FIX] log_msg używa snprintf zamiast String - eliminuje fragmentację heap
...
// Przeciążenie dla kompatybilności wstecznej z String (oznaczone jako deprecated)
inline void log_msg(int level, const String& msg) {
    log_msg(level, msg.c_str());
}

// [NEW] Bezpieczne logowanie z formatowaniem - unika alokacji String
