...
// Replace std::vector<String> with static char array
                    ...
                    case UiState::UI_STATE_MENU_PROFILES:
                        { 
                            ...
                            if (pin == PIN_BTN_UP) { profileMenuIndex = (profileMenuIndex - 1 + profileListCount) % profileListCount; }
                                ...
                                const char* selectedProfile = profileList[profileMenuIndex];
                                char path[128];
...
void ui_update_display() {
    ...
    if (force_redraw) {
        ...
        last_state_str[0] = '\0'; last_step_name[0] = '\0'; last_elapsed_str[0] = '\0'; last_remaining_str[0] = '\0';
    }
    ...
    snprintf(tempBuf, sizeof(tempBuf), "%.1f C", last_tc);
    updateText(50, 5, 78, 16, tempBuf, buf, ST77XX_ORANGE, 2);
    last_tc = tc;
    
    snprintf(buf, sizeof(buf), "%.1f C", tm);
    snprintf(tempBuf, sizeof(tempBuf), "%.1f C", last_tm);
    updateText(50, 27, 78, 16, tempBuf, buf, ST77XX_YELLOW, 2);
    last_tm = tm;
    ...
    if (st == ProcessState::RUNNING_AUTO || st == ProcessState::RUNNING_MANUAL) {
        ...
        snprintf(buf, sizeof(buf), "%.1f C", ts);
        snprintf(tempBuf, sizeof(tempBuf), "%.1f C", last_ts);
        updateText(50, 53, 78, 16, tempBuf, buf, ST77XX_CYAN, 2);
        last_ts = ts;
    } else {
    ...
    if (currentUiState == UiState::UI_STATE_IDLE) {
        if (st != ProcessState::IDLE) {
            ...
            if (st == ProcessState::RUNNING_AUTO) {
                snprintf(buf, sizeof(buf), "Krok: %s", stepName);
                updateText(0, 80, 128, 8, last_step_name, buf, ST77XX_WHITE, 1);
                strncpy(last_step_name, buf, sizeof(last_step_name) - 1);
                last_step_name[sizeof(last_step_name) - 1] = '\0';

                unsigned long elapsedSec = (millis() - stepStartTime) / 1000;
                char timeBuf[16];
                formatTime(timeBuf, sizeof(timeBuf), elapsedSec);
                snprintf(buf, sizeof(buf), "Uplynelo: %s", timeBuf);
                updateText(0, 95, 128, 8, last_elapsed_str, buf, ST77XX_WHITE, 1);
                ...
                formatTime(timeBuf, sizeof(timeBuf), remainingSec);
                snprintf(buf, sizeof(buf), "Zostalo:  %s", timeBuf);
                updateText(0, 110, 128, 8, last_remaining_str, buf, ST77XX_WHITE, 1);
                strncpy(last_remaining_str, buf, sizeof(last_remaining_str) - 1);
                ...
                char timeBuf[16];
                formatTime(timeBuf, sizeof(timeBuf), elapsedSec);
                updateText(10, 105, 120, 16, last_elapsed_str, timeBuf, ST77XX_GREEN, 2);
                strncpy(last_elapsed_str, timeBuf, sizeof(last_elapsed_str) - 1);
                last_elapsed_str[sizeof(last_elapsed_str) - 1] = '\0';
            }
        } else {
        ...
        switch (currentUiState) {
            ...
            case UiState::UI_STATE_MENU_PROFILES:
                if (profilesLoading) {
                    ...
                    // Get profile list as JSON
                    char jsonBuffer[2048];
                    size_t jsonLen;
                    if (sourceMenuIndex == 0) {
                        jsonLen = storage_list_profiles_json(jsonBuffer, sizeof(jsonBuffer));
                    } else {
                        jsonLen = storage_list_github_profiles_json(jsonBuffer, sizeof(jsonBuffer));
                    }
