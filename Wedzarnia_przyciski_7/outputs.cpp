...
void allOutputsOff() {
    output_lock();
    ledcWrite(LEDC_CHANNEL_SSR1, 0);
    ledcWrite(LEDC_CHANNEL_SSR2, 0);
    ledcWrite(LEDC_CHANNEL_SSR3, 0);
    digitalWrite(PIN_FAN, LOW);
    ledcWrite(LEDC_CHANNEL_SMOKE, 0);
    output_unlock();
}
...
// NOWE: Sprawdza czy wszystkie grzałki są gotowe
...
void mapPowerToHeaters() {
    ...
    output_lock();
    ledcWrite(LEDC_CHANNEL_SSR1, (int)(p1 * 2.55));
    ledcWrite(LEDC_CHANNEL_SSR2, (int)(p2 * 2.55));
    ledcWrite(LEDC_CHANNEL_SSR3, (int)(p3 * 2.55));
    output_unlock();
}
