// config.h - Zoptymalizowana wersja
#pragma once
#include <Arduino.h>
#include <WiFi.h>
#include <SD.h>

// ======================================================
// 1. DEFINICJE PINÓW
// ======================================================
#define PIN_ONEWIRE 4
#define PIN_SSR1 12
#define PIN_SSR2 13
#define PIN_SSR3 14
#define PIN_FAN 27
#define PIN_SMOKE_FAN 26
#define PIN_DOOR 25
#define PIN_BUZZER 33
#define PIN_BTN_UP      32 
#define PIN_BTN_DOWN    17
#define PIN_BTN_ENTER   16
#define PIN_BTN_EXIT    21
#define PIN_SD_CS 5
#define TFT_CS 15
#define TFT_DC 2
#define TFT_RST 22

// ======================================================
// 2. KONFIGURACJA GLOBALNA
// ======================================================

// --- Wyświetlacz ---
constexpr int SCREEN_WIDTH = 128;
constexpr int SCREEN_HEIGHT = 160;
#define ST77XX_DARKGREY 0x7BEF

// --- LEDC (PWM) ---
constexpr int LEDC_FREQ = 5000;
constexpr int LEDC_RESOLUTION = 8;

// --- PID ---
constexpr double CFG_Kp = 5.0;
constexpr double CFG_Ki = 0.3;
constexpr double CFG_Kd = 20.0;

// --- Limity ---
constexpr double CFG_T_MAX_SOFT = 130.0;
constexpr double CFG_T_MIN_SET = 20.0;
constexpr double CFG_T_MAX_SET = 130.0;
constexpr unsigned long CFG_MAX_PROCESS_TIME_MS = 12UL * 60UL * 60UL * 1000UL; // 12h
constexpr int CFG_SMOKE_PWM_MIN = 0;
constexpr int CFG_SMOKE_PWM_MAX = 255;
constexpr int CFG_POWERMODE_MIN = 1;
constexpr int CFG_POWERMODE_MAX = 3;

// --- Domyślne wartości ---
constexpr unsigned long CFG_FAN_ON_DEFAULT_MS = 10000;
constexpr unsigned long CFG_FAN_OFF_DEFAULT_MS = 60000;

// --- WiFi / Web ---
constexpr const char* CFG_AP_SSID = "Wedzarnia";
constexpr const char* CFG_AP_PASS = "12345678";

// --- WiFi Auto-Reconnect ---
constexpr unsigned long CFG_WIFI_CHECK_INTERVAL = 5000;
constexpr unsigned long CFG_WIFI_RETRY_MIN_DELAY = 5000;
constexpr unsigned long CFG_WIFI_RETRY_MAX_DELAY = 60000;

// --- GitHub Repo for Profiles ---
constexpr const char* CFG_GITHUB_API_URL = "https://api.github.com/repos/kudys11/wedzarnia-przepisy/contents/profiles";
constexpr const char* CFG_GITHUB_PROFILES_BASE_URL = "https://raw.githubusercontent.com/kudys11/wedzarnia-przepisy/main/profiles/";

// --- Watchdog ---
constexpr int WDT_TIMEOUT = 8; // sekundy
constexpr unsigned long SOFT_WDT_TIMEOUT = 30000; // milisekundy
constexpr unsigned long TASK_WATCHDOG_TIMEOUT = 2000; // 2 sekundy dla zadań

// --- Czujniki ---
constexpr unsigned long TEMP_REQUEST_INTERVAL = 1200;
constexpr unsigned long TEMP_CONVERSION_TIME = 850;
constexpr int SENSOR_ERROR_THRESHOLD = 3;
constexpr unsigned long SENSOR_READ_TIMEOUT = 100; // ms timeout dla czujników

// --- Stałe przypisania czujników ---
constexpr int DEFAULT_CHAMBER_SENSOR = 0;  // Pierwszy czujnik = komora
constexpr int DEFAULT_MEAT_SENSOR = 1;     // Drugi czujnik = mięso
constexpr unsigned long SENSOR_ASSIGNMENT_CHECK = 10000; // ms

// --- Profil ---
constexpr int MAX_STEPS = 8;

// --- Timeouty dla mutexów ---
constexpr TickType_t CFG_MUTEX_TIMEOUT_MS = 1000;

// --- Logging ---
constexpr int LOG_LEVEL_DEBUG = 0;
constexpr int LOG_LEVEL_INFO = 1;
constexpr int LOG_LEVEL_WARN = 2;
constexpr int LOG_LEVEL_ERROR = 3;
constexpr int CURRENT_LOG_LEVEL = LOG_LEVEL_INFO;

// --- Low Power Mode ---
constexpr unsigned long LOW_POWER_TIMEOUT = 1800000; // 30 minut

// --- Adaptive PID ---
constexpr unsigned long PID_ADAPTATION_INTERVAL = 60000; // 1 minuta

// --- Progi pamięci ---
constexpr uint32_t HEAP_WARNING_THRESHOLD = 20000;  // Ostrzeżenie przy <20KB (było 15KB)
constexpr uint32_t HEAP_CRITICAL_THRESHOLD = 10000; // Krytyczne przy <10KB

// --- Debouncing (zwiększone dla braku kondensatorów sprzętowych) ---
constexpr unsigned long DEBOUNCE_DELAY = 100; // 100ms (było domyślnie 50ms)
constexpr unsigned long LONG_PRESS_DURATION = 1000; // Długie wciśnięcie

// ======================================================
// 3. DEFINICJE TYPÓW I STRUKTUR
// ======================================================

enum class ProcessState {
    IDLE,
    RUNNING_AUTO,
    RUNNING_MANUAL,
    PAUSE_DOOR,
    PAUSE_SENSOR,
    PAUSE_OVERHEAT,
    PAUSE_USER,
    ERROR_PROFILE,
    SOFT_RESUME
};

enum class RunMode {
    MODE_AUTO,
    MODE_MANUAL
};

enum class ErrorCode {
    NONE = 0,
    SD_INIT_FAILED,
    SENSOR_DISCONNECTED,
    OVERHEAT,
    PROFILE_INVALID,
    WIFI_FAILED,
    MUTEX_TIMEOUT,
    TASK_TIMEOUT
};

struct HeaterEnable {
    bool h1, h2, h3;
    unsigned long t1, t2, t3;
};

struct Step {
    char name[32];
    double tSet;
    double tMeatTarget;
    unsigned long minTimeMs;
    int powerMode;
    int smokePwm;
    int fanMode;
    unsigned long fanOnTime;
    unsigned long fanOffTime;
    bool useMeatTemp;
};

struct ProcessStats {
    unsigned long totalRunTime;
    unsigned long activeHeatingTime;
    int stepChanges;
    int pauseCount;
    double avgTemp;
    unsigned long lastUpdate;
    unsigned long totalProcessTimeSec;
    unsigned long remainingProcessTimeSec;
};

// ======================================================
// 4. FUNKCJE POMOCNICZE
// ======================================================

// Logging z poziomami
inline void log_msg(int level, const String& msg) {
    if (level >= CURRENT_LOG_LEVEL) {
        const char* prefix[] = {"[DBG]", "[INF]", "[WRN]", "[ERR]"};
        Serial.printf("%s %s\n", prefix[level], msg.c_str());
    }
}
